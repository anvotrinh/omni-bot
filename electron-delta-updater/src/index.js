...
class DeltaUpdater extends EventEmitter {
  ...
  async prepareUpdater() {
    ...
    if (process.platform === 'win32') {
      ...
      this.deltaUpdaterRootPath = path.join(
        cachePath,
        this.updateConfig.updaterCacheDirName,
      );
      if (!fs.existsSync(this.deltaUpdaterRootPath)) {
        fs.mkdirSync(this.deltaUpdaterRootPath);
      }
    } else {
    ...
    if (app.isPackaged && process.platform === 'linux') {
      this.mksquashfsPath = path
        .join(__dirname, '..', 'linux-helpers', process.arch, './mksquashfs')
        .replace('app.asar', 'app.asar.unpacked');
      this.linuxUpdaterPath = path
        .join(__dirname, '..', 'linux-helpers', process.arch, './runtime')
        .replace('app.asar', 'app.asar.unpacked');
      this.hpatchzPath = path
        .join(__dirname, '..', 'linux-helpers', process.arch, 'hpatchz')
        .replace('app.asar', 'app.asar.unpacked');
    }
  }
  ...
  pollForUpdates(resolve, reject) {
    ...
    setInterval(() => {
      ...
      if (curHour !== 8 && curHour !== 20) return;
      this.checkForUpdates(resolve, reject);
    }, 20 * 60 * 1000);
  }
  ...
  ensureSafeQuitAndInstall() {
    ...
    browserWindows.forEach((browserWindow) => {
      browserWindow.destroyAllViews && browserWindow.destroyAllViews();
    ...
    if (this.autoUpdateInfo) {
      ...
      if (this.autoUpdateInfo.delta) {
        ...
        if (process.platform === 'darwin') {
          const command = `${this.macUpdaterPath} ${getAppName()} ${
  ...
  showUpdateNotification(info) {
    const notification = new Notification({
      title: `${getAppName()} ${
        info.version
      } is available and will be installed on exit.`,
  ...
  async applyDeltaUpdate(deltaPath, version) {
    ...
    try {
      if (process.platform === 'darwin') {
        const command = `${this.macUpdaterPath} ${getAppName()} ${deltaPath} ${
          this.hpatchzPath
        }`;
        this.logger.info(
        ...
        const command1 = `${this.hpatchzPath} ${
          this.appPath
        } ${deltaPath} ${path.join(this.deltaUpdaterRootPath, 'ss')}`;
        this.logger.info('[Updater] apply deltas ', command1);
        execSync(command1);
        const command2 = `${this.mksquashfsPath} ${path.join(
          this.deltaUpdaterRootPath,
          'ss',
        )} ${path.join(
          this.deltaUpdaterRootPath,
        ...
        this.logger.info(
          '[Updater] cretae updated appimage (step 1)',
          command2,
        );
        execSync(command2);
        const newImageName = `Qute-${version}.AppImage`;
        const command3 = `cat ${this.linuxUpdaterPath} >> ${path.join(
          this.deltaUpdaterRootPath,
          newImageName,
        )}`;
        this.logger.info(
          '[Updater] cretae updated appimage (step 2) ',
          command3,
        );
        execSync(command3);
        const command4 = `cat ${path.join(
          this.deltaUpdaterRootPath,
          'Qute.squashfs',
        )} >> ${path.join(this.deltaUpdaterRootPath, newImageName)}`;
        this.logger.info(
          '[Updater] cretae updated appimage (step 3) ',
          command4,
        );
        execSync(command4);
        const command5 = `chmod a+x ${path.join(
          this.deltaUpdaterRootPath,
          newImageName,
        )}`;
        this.logger.info(
          '[Updater] cretae updated appimage (step 4) ',
          command5,
        );
        execSync(command5);
        const command6 = `mv ${path.join(
          this.deltaUpdaterRootPath,
          newImageName,
        )} ${path.dirname(process.env.APPIMAGE)}/${newImageName}`;
        this.logger.info(
          '[Updater] cretae updated appimage (step 5) ',
          command6,
        );
        execSync(command6);
        const command7 = `ln -sf ${newImageName} ${path.dirname(
          process.env.APPIMAGE,
        )}/qute`;
        this.logger.info(
          '[Updater] cretae updated appimage (step 6) ',
          command7,
        );
        execSync(command7);
        const env = {
        ...
        try {
          const params = { stdio: 'ignore', ...env, detached: true };
          const p = spawn(
            `${path.dirname(process.env.APPIMAGE)}/${newImageName}`,
            [],
            params,
          ...
          p.unref();
          if (p.pid !== undefined) {
            this.logger.error('asasasas');
          }
        } catch (error) {
          this.logger.error(error);
        }
        //        res = this.autoUpdater.spawnLog(`${path.dirname(process.env.APPIMAGE)}/${newImageName}`, [], env);
