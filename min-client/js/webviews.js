...
var requestHideViewTimeoutId;
var hideImagePlaceholderTimeoutId;
...
const webviews = {
  ...
  q1InputUpdate: function (id, e) {
    ipc.send('q1InputUpdate', {
      id: id,
      event: e,
    });
  },
  q1Submit: function (id, e) {
    ipc.send('q1Submit', { id: id, event: e });
  },
  q1LayoutResize: function (layoutSize) {
    ipc.send('q1LayoutResize', {
      bounds: webviews.getViewBounds(),
  ...
  requestPlaceholder: function (reason, shouldCapture = true) {
    if (!reason) return;
    if (webviews.placeholderRequests.includes(reason)) return;
    webviews.placeholderRequests.push(reason);

    hideImagePlaceholderTimeoutId &&
      clearTimeout(hideImagePlaceholderTimeoutId);

    if (!shouldCapture || !webviews.selectedId) {
      hideCurrentView();
      return;
    }
    ipc.send('getCapture', { id: webviews.selectedId });
  },
  hidePlaceholder: function (reason) {
    ...
    if (webviews.placeholderRequests.length > 0) return;

    requestHideViewTimeoutId && clearTimeout(requestHideViewTimeoutId);
    ...
    hideImagePlaceholderTimeoutId = setTimeout(function () {
      if (webviews.placeholderRequests.length === 0) {
...
function hideCurrentView() {
  if (webviews.placeholderRequests.includes('logo-menu-hover')) {
    window.onImagePlaceholderLoaded && window.onImagePlaceholderLoaded();
  }
  ipc.send('hideCurrentView');
  webviews.selectedId && webviews.emitEvent('view-hidden', webviews.selectedId);
}
ipc.on('captureData', function (e, data) {
  if (webviews.placeholderRequests.length === 0) return;
  if (data.id !== webviews.selectedId) return;
  ...
  placeholderImg.onload = () => {
    requestHideViewTimeoutId && clearTimeout(requestHideViewTimeoutId);
    // wait for the image fully rendered
    requestHideViewTimeoutId = setTimeout(() => {
      hideCurrentView();
    }, 100);
  };
});
