...
const defaultKeybindings = {
  initialize: function () {
    ...
    // cmd+x should switch to tab x.
    for (let i = 1; i < 10; i++) {
      keybindings.defineShortcut({ keys: 'mod+' + i }, (e) => {
        window.switchToPlatformByNumber && window.switchToPlatformByNumber(i);
      });
    }
    ...
    keybindings.defineShortcut('switchToPreviousTab', function (d) {
      ...
      //   browserUI.switchToTab(tabs.getAtIndex(tabs.count() - 1).id);
      // }
      window.switchToPreviousPlatform && window.switchToPreviousPlatform();
    });
    ...
    keybindings.defineShortcut('reload', window.reloadCurrentTab);
