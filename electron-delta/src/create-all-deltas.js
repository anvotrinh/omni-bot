...
const createAllDeltas = async ({
  ...
  for (const { fileName, version } of previousReleases) {
    ...
    if (!fs.existsSync(path.join(extractedDir, `${processName}${platform === 'mac' ? '.app' : '.exe'}`))) {
      ...
      if(platform === 'linux' && (target === 'appimage' || target === 'AppImage')){
        fs.chmodSync(
          filePath,
          '755',
        );
        await extractAppImage(filePath, extractedDir);
      } else {
  ...
  fs.emptyDirSync(latestReleaseDir);
  if(platform === 'linux' && (target === 'appimage' || target === 'AppImage')){
    fs.chmodSync(
      latestReleaseFilePath,
      '755',
    );
    await extractAppImage(latestReleaseFilePath, latestReleaseDir);
  } else {
  ...
  for (const { version } of previousReleases) {
    ...
    const oldDir = platform === 'win' || platform === 'linux' ? path.join(dataDir, version) : path.join(dataDir, version, `${productName}.app`);
    const newDir = platform === 'win' || platform === 'linux' ? latestReleaseDir : path.join(latestReleaseDir, `${productName}.app`);
