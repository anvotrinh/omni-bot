...
const getLatestReleaseInfo = ({ artifactPaths, platform, target, arch }) => {
  const latestReleaseFilePath = artifactPaths.filter((d) => {
    ...
    if (platform === 'mac') {
      if (arch === 'arm64') {
        return d.endsWith('arm64-mac.zip');
      } else if (arch === 'x64') {
        return d.endsWith('mac.zip') && !d.endsWith('arm64-mac.zip');
      }
    }
...
const DeltaBuilder = {
  build: async ({ context, options }) => {
    ...
    const { getPreviousReleases, arch } = options;
    const buildFiles = [];
