...
const extractAppImage = (appImagePath, extractedDir) => new Promise((resolve, reject) => {
  console.log(`Extracting ${appImagePath}`);
  real_ext_path = path.join(process.cwd(), 'squashfs-root')
  console.log('Start extracting to ', real_ext_path);
  try{
    spawnSync(appImagePath, ['--appimage-extract']);
    fs.renameSync(real_ext_path, extractedDir);
    resolve(extractedDir);
  } catch (error) {
    reject(error);
  }
